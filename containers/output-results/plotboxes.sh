...
echo "set style histogram stacked gap 1" >> plot.p
echo "set boxwidth 0.9" >> plot.p
...
# plot first curve
echo "plot '$filename' using ${15}:xtic(1) title '${14}' with boxes fs pattern 1\\" >> plot.p
...
for ((i = 16; i <= $#; i+=2)); do
    ...
    echo ", '$filename' using ${column_position} title '${column_title}' with boxes fs pattern $linecount\\" >> plot.p
